...
# NOVA LISTA DE PALAVRAS-CHAVE PARA O PLANO B
...
try:
    # Atenção: Renomeei o campo ITEM_ID para 'Código do anúncio' para bater com seu Excel
    KNOWLEDGE_DF = pd.read_excel('catalogo_produtos.xlsx', dtype={'Código do anúncio': str})
    print(f">>> Banco de Dados Excel v12.1 carregado. {len(KNOWLEDGE_DF)} produtos.")
except Exception as e:
    print(f"### ERRO no Excel: {e} ###")
try:
    with open('canned_responses.json', 'r', encoding='utf-8') as f:
        CANNED_RESPONSES = json.load(f)
        print(f">>> Livro de Regras v12.1 carregado. {len(CANNED_RESPONSES)} regras.")
except Exception as e:
    print(f"### ERRO no JSON de regras: {e} ###")
...
# NOVA FUNÇÃO PARA O PLANO B
def check_for_generic_description_query(question_text):
    """Verifica se a pergunta é genérica sobre a descrição do produto."""
    normalized_question = question_text.lower()
    for keyword in GENERIC_DESCRIPTION_KEYWORDS:
        if keyword in normalized_question:
            print(f">>> PERGUNTA GENÉRICA IDENTIFICADA! Keyword: '{keyword}'. Acionando busca direta no Excel.")
            return True
    return False
...
def extract_json_from_ia_response(text):
    try:
        json_start = text.find('{'); json_end = text.rfind('}') + 1
        return json.loads(text[json_start:json_end]) if json_start != -1 and json_end != -1 else None
    except Exception: return None

def get_reply_logic(question_text, product_data):
    print(">>> CÉREBRO DE IA v12.1 (Com Discernimento) ACIONADO...")
    description_text = product_data.pop('DESCRIPTION', 'Nenhuma descrição fornecida.')
    structured_data_text = "\n".join([f"{key.replace('_', ' ').title()}: {value}" for key, value in product_data.items()])
    ...
    try:
        ...
        ia_response_string = response.json()['candidates'][0]['content']['parts'][0]['text']
        print(f">>> IA (Com Discernimento) RETORNOU STRING: {ia_response_string}")
        return extract_json_from_ia_response(ia_response_string)
    except Exception as e:
...
def get_product_context_from_dataframe(item_id, dataframe):
    ...
    # ATENÇÃO: Corrigido para usar a coluna certa do seu Excel
    product_row = dataframe[dataframe['Código do anúncio'] == item_id]
    if not product_row.empty:
        # Convertendo a linha para dicionário e mantendo os nomes das colunas
        return product_row.iloc[0].dropna().to_dict()
    return None

def get_access_token():
    url = "https://api.mercadolibre.com/oauth/token"
    payload = {'grant_type': 'client_credentials', 'client_id': APP_ID, 'client_secret': CLIENT_SECRET}
    response = requests.post(url, headers={'accept': 'application/json', 'content-type': 'application/x-www-form-urlencoded'}, data=payload)
...
def post_answer(question_id, answer_text, token):
    ...
    payload = json.dumps({"question_id": question_id, "text": answer_text})
    response = requests.post(url, headers={'Authorization': f'Bearer {token}', 'Content-Type': 'application/json'}, data=payload)
...
def handle_notification():
    ...
    PROCESSED_QUESTIONS = {qid: t for qid, t in PROCESSED_QUESTIONS.items() if current_time - t < MEMORY_DURATION_SECONDS}
    ...
    if notification_data.get('topic') == 'questions':
        ...
        if question_id_from_resource in PROCESSED_QUESTIONS:
            print(f"--- NOTIFICAÇÃO DUPLICADA... IGNORANDO. ---")
            return jsonify({"status": "ignored_duplicate"}), 200
        
        PROCESSED_QUESTIONS[question_id_from_resource] = current_time
        print(f"--- INICIANDO FLUXO DE RESPOSTA PARA: {resource_id} ---")
        ...
        if access_token:
            ...
            if question_details and question_details.get('status') == 'UNANSWERED':
                ...
                item_id = question_details.get('item_id')
                if not item_id:
                    print(f"--- ERRO: A notificação {resource_id} não contém um 'item_id'.")
                    return jsonify({"status": "error_no_item_id"}), 200
                ...
                if canned_answer:
                    if post_answer(question_details.get('id'), canned_answer, access_token):
                        send_notification_email(question_text, canned_answer)
                    return jsonify({"status": "success_canned"}), 200

                # --- NOVO FLUXO 1.5: PERGUNTAS GENÉRICAS SOBRE DESCRIÇÃO (PLANO B) ---
                ...
                if check_for_generic_description_query(question_text):
                    if product_data_dict and 'Descrição' in product_data_dict:
                        description = product_data_dict['Descrição']
                        greeting = get_time_based_greeting()
                        closing = f"\n\nAguardamos sua compra!\nEquipe {YOUR_STORE_NAME}"
                        # Resposta formatada usando a descrição do Excel
                        final_answer_text = f"{greeting}Claro! Seguem os detalhes do produto: \n\n{description}{closing}"
                        
                        print(f">>> PLANO B: Enviando descrição do Excel como resposta.")
                        if post_answer(question_details.get('id'), final_answer_text, access_token):
                            send_notification_email(question_text, final_answer_text)
                        return jsonify({"status": "success_description_fallback"}), 200
                    else:
                        print(f">>> PLANO B FALHOU: Pergunta genérica, mas produto {item_id} não tem descrição no Excel.")
                ...
                if product_data_dict:
                    ...
                    print(f">>> FALHA CRÍTICA: Produto {item_id} não encontrado no banco de dados Excel.")

                if ia_response_data and isinstance(ia_response_data, dict):
                    status = ia_response_data.get('status')
                    confidence = ia_response_data.get('confidence_score', 0)
                    core_answer = ia_response_data.get('answer_text')
                    if status == "ANSWER_FOUND" and confidence >= CONFIDENCE_THRESHOLD:
                        greeting = get_time_based_greeting()
                        closing = f"\n\nAguardamos sua compra!\nEquipe {YOUR_STORE_NAME}"
                        final_answer_text = f"{greeting}{core_answer}{closing}"
                        print(f">>> DISCERNIMENTO (Status: {status}, Confiança: {confidence}/{CONFIDENCE_THRESHOLD}). Enviando resposta.")
                        if post_answer(question_details.get('id'), final_answer_text, access_token):
                            send_notification_email(question_text, final_answer_text)
                    else:
                        print(f">>> DISCERNIMENTO (Status: {status}, Confiança: {confidence}). Nenhuma ação.")
                else:
                    print(">>> FLUXO DA IA NÃO RETORNOU DADOS VÁLIDOS. Nenhuma ação será tomada.")

    return jsonify({"status": "success"}), 200

@app.route('/', methods=['GET'])
def health_check():
    return jsonify({"status": "running", "service": f"Mercado Livre IA Responder v12.1 - Supervisor ({YOUR_STORE_NAME})"}), 200

if __name__ == '__main__':
    # Usando a porta que o Render espera
